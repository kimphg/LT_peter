...
//    mSpeedkmh  = 0;
    ...
    lastUpdateTimeMs      = newobject.timeMs;
    //
...
void C_SEA_TRACK::checkNewObject()
{
    ...
    //
    lastUpdateTimeMs = possibleObj.timeMs;
    //change track status
...
void C_SEA_TRACK::update()
{
    ...
    if(CConfig::time_now_ms-mAisMaxPosibilityTimeMs>AIS_UPDATE_TIME)
    {
...
void C_radar_data::addDetectionZone(double x, double y, double dazi, double drg,bool isOneTime)
{
    ...
    dw.xkm=x;
    dw.ykm=y;
    ConvkmxyToPolarDeg(x,y,&dw.aziDeg,&dw.rg);
    dw.maxDazDeg = (dazi);
    dw.maxDrg = drg;
    addDetectionZone(dw);
...
void C_radar_data::addFreeObj(object_t *obj1)
{
    ...
    if(full&&mFreeObjList.size()<MAX_TRACKS_COUNT)
    {
